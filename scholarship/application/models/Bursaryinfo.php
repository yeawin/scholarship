 ...
 *
 */
class Application_Model_Bursaryinfo extends Zend_Db_Table_Abstract
{
    ...
    public function is_scholarship_name_exist($scholarship_name)
    {
        $select = $this->select();
        $select->from($this->_name);
        $select->where("scholarship_name = ?", $scholarship_name);
        $result = $this->fetchAll($select)->toArray();
        return (count($result) > 0);
    }
